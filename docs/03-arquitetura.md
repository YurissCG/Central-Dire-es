...
| Dependência de runtime | somente next, react, lucide-react, clsx, tailwind-merge, `motion` (reintroduzida no refresh visual de 2026-08-18, ver nota) |
...
**Nota sobre a reintrodução do `motion`, refresh visual de 2026-08-18:** a lib havia sido removida na Tarefa 7 (ver nota acima). O cliente pediu mais movimento no site (referência: Unlumen UI), então `motion` voltou para animar o menu do header, o accordion de dúvidas, hover na grade de serviços e entrada `whileInView` nas seções, sempre respeitando `prefers-reduced-motion` via `MotionConfig`. Custo pesado antes: medido com `curl` e gzip em cada chunk JS referenciado pela home, servidor de produção local (`npm run build && npm run start`), mesmo método da Tarefa 7: **~239KB comprimidos reais** para navegador moderno. Acima do piso anterior de 157KB, aumento aceito pelo dono do projeto em troca do movimento pedido; LCP continua abaixo de 2,5s (ver Tarefa 13 e auditoria final desta rodada).
